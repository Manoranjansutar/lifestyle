...
function SampleNextArrow(props) {
  ...
  function SamplePrevArrow(props) {
    ...
    return (
      <div
        className={className}
        style={{ ...style, display: "block", background: "black" , width:'35px' , height:'35px', borderRadius:'50%',padding:'7px' , position:'relative' , top:'193px',left:'-15px',marginRight:'20px' ,zIndex:'1'}}
        onClick={onClick}
      />
    );
  }
  ...
  return (
    <div className='trending2'>
          <Slider {...settings} className='trending-slider'>
         {
                ...
                if (item.category === "Women" && item.rating >= 4.5){
                    return(
                        <div key={index} className='trending2-items'>
                             <ItemSlider
                      key={index}
                      image={item.image}
